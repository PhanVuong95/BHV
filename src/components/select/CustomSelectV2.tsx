...
const CustomSelectV2 = (props) => {
  // const [focus, setFocus] = useState(false);
  const { label, value, onChange, rules, options = [], name, form } = props;
    ...
    <>
      ...
      >
        <Form.Item name={name} rules={rules}>
          ...
          >
            {findValueInOptions?.label}
          </div>
          <label className={labelClass}>{label}</label>
          {/* {error && <div className="ant-form-item-explain-error">Không được để trống</div>} */}
          <svg
        ...
        {options?.map((itemMap, key, arr) => (
          <>
            <div
              onClick={() => {
                onChange(itemMap);
                form.setFieldValue(name, itemMap?.value);
                form.setFields([
                  {
                    name,
                    errors: [],
            ...
            >
              {itemMap?.label}
              {itemMap?.label === findValueInOptions?.label && (
                <svg
                  className="custom-icon-right"
                  style={{ right: "0px" }}
                  width="16"
