...
export const ExtendedAccidentInsuranceFormConfirmData = ({
  insuranceData,
}: {
  insuranceData: ICreateExtendedAccidentInsuranceParams;
}) => {
  const [bicExtendedAccidentCategory, setbicExtendedAccidentCategory] =
    useState<ICategoryResponse[]>([]);

  useEffect(() => {
    getInsuranceCategory(ECategory.EXTENDED_ACCIDENT_PACKAGES, {
      source: EInsuranceSource.BIC,
    }).then((res) => setbicExtendedAccidentCategory(res));
  }, []);

  const travelCategory = useMemo(
    () =>
      ({
        [EInsuranceSource.BIC]: bicExtendedAccidentCategory,
      }[insuranceData.source]),
    [bicExtendedAccidentCategory, insuranceData.source]
  );
